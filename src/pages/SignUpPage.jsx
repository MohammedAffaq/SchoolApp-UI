  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      try {
        ...
        let registeredUsers = existingData ? JSON.parse(existingData) : {};

        // Check if admin already exists
        if (registeredUsers.admin) {
          setErrors({ general: 'Admin account already exists. Only one admin can be registered.' });
          setLoading(false);
          return;
        }
        ...
        registeredUsers.admin = newAdmin;
        ...
        setErrors({ general: 'Failed to save registration. Please try again.' });
      } finally {
        setLoading(false);
      }
    }, 1000);
